...
export default function StarOnGithub() {
  ...
  useEffect(() => {
    ...
    const fetchStarCount = async () => {
      try {
        const response = await fetch('https://api.github.com/repos/AnirudhSingh07/dapp-analytics-platform');
        if (response.ok) {
  ...
  const handleStarClick = () => {
    // Track the click event (can be integrated with your analytics system)
    ...
    window.open('https://github.com/AnirudhSingh07/dapp-analytics-platform', '_blank');
  };
